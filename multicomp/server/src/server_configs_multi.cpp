...
void addr_init(SOCKADDR_IN& addr, int port)
{
	...
	addr.sin_port = htons(port); // sever Port
...
int SendArrayToClient_bicomp(SOCKET& Connection, vector<DTYPE>& arr_full, LARGE_INTEGER start)
{
	...
	while (remainBytes > 0)
	{
		...
		const char* dataToSend = reinterpret_cast<const char*>(arr_full.data()) + 
			(SORT_DATANUM * sizeof(DTYPE) - remainBytes); 
...
// port_idx = 0/1/2
// self(-1):0~DATANUM/4-1 0:DATANUM/4~DATANUM/4*2-1
// 1:DATANUM/4*2~DATANUM/4*3-1  2:DATANUM/4*3~DATANUM-1

int SendArrayToClient_multicomp(SOCKET& Connection, vector<DTYPE>& arr_full, int port_idx)
{
	const size_t sect_datanum = DATANUM / 4;
	if (Connection == 0)
	{
		std::cout << "bad connection." << std::endl;
		return -1;
	...
	int remainBytes = sect_datanum * sizeof(DTYPE);
	while (remainBytes > 0)
	{
		int BytesToSend = min(SEND_RECV_BUFFER_SIZE, remainBytes);

		const char* dataToSend = reinterpret_cast<const char*>(arr_full.data()) + 
			(port_idx + 1) * sect_datanum * sizeof(DTYPE) + 
			(sect_datanum * sizeof(DTYPE) - remainBytes);
...
vector<DTYPE> RecvArrayBackFromClient_multicomp(SOCKET& Connection)
{
	const size_t sect_datanum = DATANUM / 4;
	char recvArrBuffer[SEND_RECV_BUFFER_SIZE];
	...
	*checkbuff = '\0';
	...
	char* arr_recv_tmp = new char[(sect_datanum + 1) * sizeof(DTYPE)];
	size_t recv_len = 0;
	size_t bytesRecv = 0;
	while (recv_len < sect_datanum * sizeof(DTYPE))
	{
		memset(recvArrBuffer, 0, SEND_RECV_BUFFER_SIZE);

		int bytesRecv = recv(Connection, recvArrBuffer, SEND_RECV_BUFFER_SIZE, NULL);
		if (recv_len == sect_datanum * sizeof(DTYPE))
		{
			break;
		}
		else if (bytesRecv < 0)
	...
	vector<DTYPE> arr_recv(floatData, floatData + sect_datanum);
